...
<body style="background-color: #fafafa">
	...
	<table class="table">
	<tr><td><h4>직원:</h4> </td>
		<td><input type="hidden" name="name" value=""/><div class="col-xs-4"><select class="form-control" name="employee_number">
				<option disabled="disabled">----------개발부----------</option>
				<c:forEach items="${develList}" var="devList">
				<option value="${devList.employee_number}">${devList.employee_number}  ${devList.getName()} ${devList.getPosition()}</option>
				</c:forEach>
				<option disabled="disabled">---------디자인부---------</option>
				<c:forEach items="${designList}" var="desList">
				<option value="${desList.employee_number}">${desList.employee_number} ${desList.getName()} ${desList.getPosition()}</option>
				</c:forEach>
			</select></div></td></tr>
	<tr><td><h4>프로젝트 번호:</h4> </td>
		<td><h4>${project_number}<input type="hidden" name="project_number" value="${project_number}"></h4></td></tr>
		
	<tr><td><h4>투입일:</h4></td>
		<td><div class="col-xs-3"><input type="text" name="assign_date" class="form-control"OnClick="Calendar(this, 'down','no');" value="2015-12-03"></div></td></tr>
	<tr><td><h4>투출일:</h4> </td>
		<td><div class="col-xs-3"><input type="text" name="abort_date" class="form-control"OnClick="Calendar(this, 'down','no');" value="2015-12-03"></div></td></tr>
	...
	<tr><td><h4>직책:</h4> </td>
		<td><div class="col-xs-3"><select class="form-control"name="job">
			...
			<option value="04">설계자</option>
			<option value="05">프로그래머</option>
			<option value="06">테스터</option>
			<option value="07">디자이너</option>
		</select></div></td></tr>
		
	<tr><td><button type="submit" class="btn btn-default"value="투입">투입</button></td></tr>
	...
	<tr><td><h4>직원 리스트:</h4> </td><td>
		<td><div><select class="form-control"name="employee_number">
		<option selected="selected">선택하세요</option>
	<c:forEach items="${assign}" var="assignment">
			<option value="${assignment.employee_number}">${assignment.employee_number}</option>
		</c:forEach>
			</select></div></td></tr>
	<tr><td><input type="hidden" name="project_number" value="${project_number}"/>
		<button type="submit" class="btn btn-default" value="투출">투출</button></td></tr>
	</table>
	</form>
